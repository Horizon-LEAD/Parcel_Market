...
def run_module_cs(varDict):
    ...
    # calculated
    # FIXME: should the constants or coefficients be configurable ?
    # varDict["CS_Willingess2Send"] = "3.45 + (Cost-TradCost) * (-0.3292)"
    varDict["CS_Willingess2Send"] = "0.01 + (Cost-TradCost) * (-0.3292)"
    varDict['CS_BringerUtility'] = "0.01 + (-0.039) * Cost + (-0.0063) * Time"
    ...
    def generate_CS_supply(trips, CS_willingness,CS_BringerFilter): # CS_willingness is the willingness to be a bringer
        ...
        for traveller in trips_array:
            ...
            if mode == 'Car':
                ...
                traveller[28] = generate_Utility(
                    varDict['CS_BringerUtility'], {'Cost': cost, 'Time':traveller[24]}
                )
   ...
   #%% Checking whether the parcels will use CS with a choice model
    ...
    parcels['CS_deliveryChoice'] = parcels.apply(
        lambda x: generate_Utility(varDict['CS_Willingess2Send'],
                                   {'Cost': x.cost,'TradCost' : varDict['TradCost']}),
        axis=1
    )
        ...
        for index, parcel in parcels.iterrows():
            ...
            if parcel['CS_deliveryChoice']:
                ...
                for i, traveller in filtered_trips.iterrows():
                    ...
                    Util_PickUp = generate_Utility(
                        varDict['CS_BringerUtility'], {'Cost': NetCompensation,'Time':CS_trip_time}
                    ) #+80# This is a provisional number so there aren't that many parcels that are eligible until we find the correct equation
